...
class Node:
    ...
    def __str__(self) -> str:
        return f"Node<pos: {round(self.position[0], 4)}, {round(self.position[1], 4)}, #neighbors: {len(self.neighbors)}>"
...
class RrtPlanner:
    ...
    def generate_plan(
        self, start: POSITION_TYPE, goal: POSITION_TYPE
    ) -> Tuple[List[POSITION_TYPE], List[Node]]:
        ...
        for _ in range(max_iterations):
            ...
            if (
                np.linalg.norm(q_new.position - goal_node.position)
                <= self.goal_threshold
            ):
                goal_reached = q_new
                break
        ...
        if goal_reached is None:
            dists_to_goal = [
                np.linalg.norm(node.position - goal_node.position) for node in graph
            ]
        ...
        if (
            len(plan) == 0
            or np.linalg.norm(
                goal_node.position - np.array([plan[-1]["x"], plan[-1]["y"]])
            )
            > 1e-3
        ):
