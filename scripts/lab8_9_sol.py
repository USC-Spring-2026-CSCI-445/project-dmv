...
class Controller:
    ...
    def take_measurements(self):
        ...
        indices = [
            max(0, min(num_ranges - 1, mid_idx + i)) for i in (-offset, 0, offset)
        ]
        measurements = []
    ...
    def autonomous_exploration(self):
        ...
        for step in range(max_steps):
            ...
            if self.laserscan is not None:
                ...
                front_sector = [
                    r for r in ranges[low_idx : high_idx + 1] if not np.isinf(r)
                ]
            ...
            if too_close:
                ...
                self.rotate_action(
                    uniform(math.pi / 5, math.pi / 3)
                )  # bigger rotation away
                rotation_attempts += 1
                rate.sleep()
                continue
            ...
            if pts.shape[0] > 0:
                ...
                if std_dev < 0.12 and sensor_ok:
                    rospy.loginfo(
                        "Particle filter converged (std < 0.12 and sensor matched)."
                    )
                    break
    ...
    def forward_action(self, distance: float):
        ...
        r = rospy.Rate(10)
        while (rospy.Time.now().to_sec() - start) < duration and (
            not rospy.is_shutdown()
        ):
